...
import gpt_icon from "./gpt_icon.png";
...
const SuggestConceptPage = () => {
  ...
  const updateShortDefinition = (response) => {
    if (response && response.shortDefinition && response.shortDefinition.english) {
      setShortDefinition((prevShortDefinition) => ({
        ...prevShortDefinition,
  ...
  const handleChatGptButtonClick = async () => {
    ...
    try {
      ...
      const conceptResponse = await GptApi.sendRequest(userInput);
      console.log('asdf', conceptResponse);
      
      updateStateWithResponse(conceptResponse);
    } catch (error) {
 ...
 const updateStateWithResponse = (response) => {
  ...
  if (!response.conceptName) {
    console.error("conceptName is missing");
    return;
  }

  if (!response.conceptName.english) {
    console.error("conceptName.english is missing");
    return;
  }

  if (!response.longDefinition) {
    console.error("longDefinition is missing");
    return;
  }

  if (!response.longDefinition.english) {
    console.error("longDefinition.english is missing");
    return;
  }

  if (!response.shortDefinition) {
    console.error("shortDefinition is missing");
    return;
  }

  if (!response.shortDefinition.english) {
    console.error("shortDefinition.english is missing");
    return;
  }

  console.log("Valid response. Updating state...");
  setConceptName(response.conceptName.english);
  
  console.log("Updated conceptName state with value:", response.conceptName.english);
  setLongDefinition(response.longDefinition.english);
  console.log("the state" , longDefinition);
  console.log("Updated longDefinition state with value:", response.longDefinition.english);
  setShortDefinition((prevShortDefinition) => ({
    ...prevShortDefinition,
    english: response.shortDefinition.english,
  }));
  console.log("Updated shortDefinition state with value:", response.shortDefinition.english);
  
  
  
  
};
console.log("hoaishdoa" , longDefinition);
console.log(shortDefinition.english);
